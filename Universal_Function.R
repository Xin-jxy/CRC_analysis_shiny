...
###### The input required is the name of samples, which exist in RSEM file, and the normalized_method is either "FPKM" or "TPM"
normData<-function(charctere_pret,normalized_method){
  ...
  for (i in 1:length(charctere_pret)){
    ...
    new_data[[sampleNames[i]]]=list_data[[sampleNames[i]]][[normalized_method]]
...
#### The file import should have ENSEMBL ID as rownames, the output data is the file with SYMBOL ID###
...
rownames(data_pret)=gsub("\\.","-",rownames(data_pret))

return(data_pret)
}

####### This function is a function of anntotation for heatmap (with or without Cov7)  ########
####### example: sample_info_commun=Fun_annot("withoutCOV7"), you can get a dataframe which has annotation for the semples who don't contain the sample COV7
Fun_annot<-function(sampleNames){
  if (sampleNames=="withCOV7"){
    #within COV7
    histology=c(rep("adenocarcinoma",4),"adenosquamous carcinoma","squamous cell carcinoma","carcinome neuroendocrine",
    ...
    age=c("40-60",rep("60-80",2),"40-60",rep("60-80",2),"30","40-60",rep("60-80",4),rep("40-60",2),rep("60-80",9))
    sexe=c(rep("M",2),rep("F",2),rep("M",2),"F","M",rep("F",2),rep("M",4),"F",rep("M",7),"F")
...
####################################################################################################
#########  This part of function if mainly for Shiny, but could be useful for analyse as well ######
####################################################################################################

############### This function used for loading the normalized data (part of shiny, normal data analyse doesn't involve) #############
Fun_loadNormFile<-function(dataNorm){
  #load normalization data
    if (dataNorm=="Deseq2"){
      normalized_counts=read.csv("~/Documents/analysis_script_RNAseq/data/normalized_medianOfRatios_Deseq2.csv",header=T,row.names=1)
    }
    else if (dataNorm=="edgeR"){
      normalized_counts=read.csv("~/Documents/analysis_script_RNAseq/data/normalized_TMM_edgeR.csv",header=T,row.names=1)
    }
    else{
...
#add annotation for up and downregulated
  ...
  Heatmap(scale_data_deseq,name = "Z-score",cluster_rows=F,top_annotation=annot_degs,left_annotation=annot_row_degs_deseq2,
          column_split = sample_info_commun$Groups,column_title = "Heatmap of DEGs genes")
...
######################## This function is for the cnetplot ######################################
...
Fun_genelist<-function(dataNames){
  if (dataNames=="Deseq2"){
    ...
    results.DESeq2=read.csv("~/Documents/analysis_script_RNAseq/data/StatRes_EdgeR.csv",header=T,row.names=1)
  }
  else{
    message("error of input the normalized data")
  } 
...
######################This function is for MCPcounter TPM normalisation data, get FPKM normalization, 
...
Fun_violin<-function(df_cov7,covid,negatif){
  split_cellType=df_cov7 %>% group_split(Cell_Type)
  
  a=length(unique(df_cov7$Cell_Type))
  p <- list()
  for(i in 1:a){
    p[[i]]=ggplot(split_cellType[[i]],aes(x=Sample,y=value,fill=Sample))+geom_violin(trim=F)+scale_fill_manual(values=c("#FFC20A","#0C7BDC"))+
      geom_signif(comparisons = list(c(covid,negatif)),test = "wilcox.test",map_signif_level = TRUE,textsize=25, y_position=c(max(split_cellType[[i]]$value)-0.2,max(split_cellType[[i]]$value),max(split_cellType[[i]]$value)+0.3))+
      theme_classic()+theme(legend.position = "none",axis.title.x = element_blank(),axis.text.x=element_text(size=rel(3)),plot.title = element_text(size = 30))+
